...
public class RepositorySupport implements SQLTransactionManager {
    ...
    private static <T, U> boolean isElementsEquals(AtomicStorableValueComponent<T, U> definition, T value1, T value2) {
    ...
    public <K, V> K putNewEntry(AutogeneratedKeyIndexedRepositoryConfiguration<K, V> configuration, V attributes) throws SQLException {
        ...
        List<? extends AtomicStorableValueComponent<V, ?>> elements = configuration.getValueDefinition().getAtomicComponents();
        ...
        try (PreparedStatement statement = connection.prepareStatementWithAutogeneratedKeys(sql)) {
            ...
            for (AtomicStorableValueComponent<V, ?> element: elements)
    ...
    public <K, V, R, E extends Exception> R get(IndexedRepositoryConfiguration<K, V> configuration, K key, OptionalVisitor<V, R, E> optionalVisitor)
        ...
        List<? extends AtomicStorableValueComponent<K, ?>> keyElements = configuration.getKeyDefinition().getAtomicComponents();
        ...
        try (PreparedStatement statement = connection.prepareStatement(sql)) {
            StatementSetter statementSetter = new StatementSetter(statement);
            for (AtomicStorableValueComponent<K, ?> element: keyElements)
    ...
    public <K, V, O> List<V> entryList(ReadableRepositoryDirectoryConfiguration<K, V, O> configuration, K key, SlicingQuery<O> slicing) throws SQLException {
        List<? extends AtomicStorableValueComponent<O, ?>> orderingElements = configuration.getOrderingDefinition().getAtomicComponents();
        List<? extends AtomicStorableValueComponent<K, ?>> keyElements = configuration.getKeyDefinition().getAtomicComponents();
        SQLBuilder sqlBuilder = new SQLBuilder();
    ...
    public <K> boolean remove(RepositoryIndex<K> configuration, K key) throws SQLException {
        ...
        try (PreparedStatement statement = connection.prepareStatement(sql)) {
            StatementSetter statementSetter = new StatementSetter(statement);
            for (AtomicStorableValueComponent<K, ?> element: configuration.getKeyDefinition().getAtomicComponents())
                statementSetter.setElement(element, key);
    ...
    public <K, V> boolean putIfExists(IndexedRepositoryConfiguration<K, V> configuration, K key, Changes<V> attributes) throws SQLException {
        ...
        List<? extends AtomicStorableValueComponent<V, ?>> valueElements = configuration.getValueDefinition().getAtomicComponents();
        for (AtomicStorableValueComponent<V, ?> element: valueElements) {
    ...
    private static class StatementSetter {
        ...
        public <T, U> void setElement(AtomicStorableValueComponent<T, U> definition, T value) throws SQLException {
    ...
    private static class VoidStorableClass implements StorableTypeDefinition<Void> {
        @Override
        public List<AtomicStorableValueComponent<Void, ?>> getAtomicComponents() {
